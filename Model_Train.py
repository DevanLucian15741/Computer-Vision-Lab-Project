...
def FaceRecognition_Training():
    ...
    for i, Folder in enumerate(train_dir_List):
        ...
        for Image in os.listdir(Data_path_Folder)[:-1]:
            ...
            for (x,y,w,h) in Extract_Faces:
                    ...
                    faces = cv2.GaussianBlur(faces, (11,11),5.0)
                    # faces = cv2.resize(faces, (500, 500 ), interpolation = cv2.INTER_AREA)
    ...
    X_train, X_test, y_train, y_test = train_test_split(image_list, Labels_List, test_size=0.25, random_state=np.random.randint(100))
