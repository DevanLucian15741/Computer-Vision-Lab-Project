...
def FaceRecognition_Training():
    ...
    X_train_resized = [cv2.resize(image, (69, 69), interpolation = cv2.INTER_AREA) for image in X_train]
    X_test_resized = [cv2.resize(image, (69, 69), interpolation = cv2.INTER_AREA) for image in X_test]
...
# Test Model
    ...
    print(f'Avarage Accuracy: {(num_correct / len(X_test_resized)) * 100.055}%')
